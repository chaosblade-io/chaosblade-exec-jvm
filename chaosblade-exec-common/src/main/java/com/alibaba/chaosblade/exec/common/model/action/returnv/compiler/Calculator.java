...
public abstract class Calculator {
    ...
    Constant gt(Constant valueA, Constant valueB) throws CompilerException {
        if (valueA.isNUMERIC() && valueB.isNUMERIC()) {
            return Constant.build(BOOLEAN, valueA.getAsNumber().doubleValue()>valueB.getAsNumber().doubleValue());
    ...
    Constant modulo(Constant valueA, Constant valueB) throws CompilerException {
        if (valueA.isNUMERIC() && valueB.isNUMERIC()) {
            return Constant.build(NUMERIC, valueA.getAsNumber().longValue() % valueB.getAsNumber().longValue());
        }
        return Constant.build(NUMERIC, Double.NaN);
    }
    ...
    Constant execute(String function, List<Constant> parameters) throws CompilerException {
        if (MIN_FUNCTION.equals(function)) {
            ...
            throw new CompilerException("min function do not accept non-numeric and no-string parameters.");
            ...
            throw new CompilerException("max function do not accept non-numeric and no-string parameters.");
            ...
            if (parameters.size()==3 && allMatchNonNull(parameters)) {
                ...
                Matcher match = Pattern.compile(parameters.get(0).getAsString())
            ...
            if (parameters.size() == 1) {
                if (parameters.get(0).isNUMERIC()) {
                    return Constant.build(BOOLEAN, !Double.isNaN(parameters.get(0).getAsNumber().doubleValue()));
                }
