 ...
 * <h3>Tick Duration</h3>
 * <p>
 * As described with 'approximated', this timer does not execute the scheduled {@link TimerTask} on time.
 * {@link HashedWheelTimer}, on every tick, will check if there are any {@link TimerTask}s behind the schedule and
 * execute them.
 * <p>
 ...
 * <p>
 * {@link HashedWheelTimer} maintains a data structure called 'wheel'. To put simply, a wheel is a hash table of
 * {@link TimerTask}s whose hash function is 'dead line of the task'. The default number of ticks per wheel (i.e. the
 * size of the wheel) is 512. You could specify a larger value if you are going to schedule a lot of timeouts.
 *
 * <h3>Do not create many instances.</h3>
 * <p>
 * {@link HashedWheelTimer} creates a new thread whenever it is instantiated and started. Therefore, you should make
 * sure to create only one instance and share it across your application. One of the common mistakes, that makes your
 * application unresponsive, is to create a new instance for every connection.
 *
 * <h3>Implementation Details</h3>
 * <p>
 * {@link HashedWheelTimer} is based on <a href="http://cseweb.ucsd.edu/users/varghese/">George Varghese</a> and Tony
 * Lauck's paper, <a href="http://cseweb.ucsd.edu/users/varghese/PAPERS/twheel.ps.Z">'Hashed and Hierarchical Timing
 * Wheels: data structures to efficiently implement a timer facility'</a>. More comprehensive slides are located
...
public class HashedWheelTimer implements Timer {
     ...
     *
     * @param threadFactory
     *            a {@link ThreadFactory} that creates a background {@link Thread} which is dedicated to
     *            {@link TimerTask} execution.
     * @throws NullPointerException
     *             if {@code threadFactory} is {@code null}
     */
     ...
     *            the time unit of the {@code tickDuration}
     * @throws NullPointerException
     *             if either of {@code threadFactory} and {@code unit} is {@code null}
     * @throws IllegalArgumentException
     *             if {@code tickDuration} is &lt;= 0
     */
     ...
     *            The maximum number of pending timeouts after which call to {@code newTimeout} will result in
     *            {@link RejectedExecutionException} being thrown. No maximum pending timeouts limit is assumed if this
