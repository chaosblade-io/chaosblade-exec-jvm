package com.otel.business.c.exec.common.context;

import com.otel.business.c.exec.common.util.ClassUtils;
import java.util.*;
 ...
 * <h3>Tick Duration</h3>
 *
 * <p>As described with 'approximated', this timer does not execute the scheduled {@link TimerTask}
 * on time. {@link HashedWheelTimer}, on every tick, will check if there are any {@link TimerTask}s
 ...
 *
 * <p>{@link HashedWheelTimer} maintains a data structure called 'wheel'. To put simply, a wheel is
 * a hash table of {@link TimerTask}s whose hash function is 'dead line of the task'. The default
 * number of ticks per wheel (i.e. the size of the wheel) is 512. You could specify a larger value
 * if you are going to schedule a lot of timeouts.
 *
 * <h3>Do not create many instances.</h3>
 *
 * <p>{@link HashedWheelTimer} creates a new thread whenever it is instantiated and started.
 * Therefore, you should make sure to create only one instance and share it across your application.
 * One of the common mistakes, that makes your application unresponsive, is to create a new instance
 * for every connection.
 *
 * <h3>Implementation Details</h3>
 *
 * <p>{@link HashedWheelTimer} is based on <a href="http://cseweb.ucsd.edu/users/varghese/">George
...
public class HashedWheelTimer implements Timer {
  ...
  private final Queue<HashedWheelTimeout> cancelledTimeouts =
      new LinkedBlockingQueue<HashedWheelTimeout>();
   ...
   * @param unit the time unit of the {@code tickDuration}
   * @param ticksPerWheel the size of the wheel
   * @param maxPendingTimeouts The maximum number of pending timeouts after which call to {@code
   *     newTimeout} will result in {@link RejectedExecutionException} being thrown. No maximum
   *     pending timeouts limit is assumed if this value is 0 or negative.
   * @throws NullPointerException if either of {@code threadFactory} and {@code unit} is {@code
   *     null}
   * @throws IllegalArgumentException if either of {@code tickDuration} and {@code ticksPerWheel} is
   *     &lt;= 0
  ...
  protected void finalize() throws Throwable {
    try {
      ...
      // This object is going to be GCed and it is assumed the ship has sailed to do a proper
      // shutdown. If
  ...
  public Timeout newTimeout(TimerTask task, long delay, TimeUnit unit) {
    ...
    // During processing all the queued HashedWheelTimeouts will be added to the correct
    // HashedWheelBucket.
  ...
  private static final class HashedWheelTimeout implements Timeout {
    ...
    /**
     * This will be used to chain timeouts in HashedWheelTimerBucket via a double-linked-list. As
     * only the workerThread will act on it there is no need for synchronization / volatile.
     */
    ...
    public boolean cancel() {
      ...
      // If a task should be canceled we put this to another queue which will be processed on each
      // tick.
      // So this means that we will have a GC latency of max. 1 tick duration which is good enough.
      // This way
      // we can make again use of our MpscLinkedQueue and so minimize the locking / overhead as much
      // as possible.
    ...
    public String toString() {
      ...
      StringBuilder buf =
          new StringBuilder(192).append(simpleClassName).append('(').append("deadline: ");
      if (remaining > 0) {
